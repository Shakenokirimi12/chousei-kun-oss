...
export function AdminEventSettings({
    ...
    const [selectedPeriods, setSelectedPeriods] = useState(
        initialCandidates.filter((candidate) => /_P\d+$/.test(candidate))
    );
    ...
    const sortedCandidates = useMemo(() => {
        return [...selectedPeriods].sort((a, b) => {
            ...
            const idA = Number(slotA.replace("P", ""));
            const idB = Number(slotB.replace("P", ""));
            const startA = CUSTOM_PERIODS.find((x) => x.id === idA)?.time.split("-")[0] ?? "00:00";
            const startB = CUSTOM_PERIODS.find((x) => x.id === idB)?.time.split("-")[0] ?? "00:00";
    ...
    const onSubmit = async (e: React.FormEvent) => {
        ...
        if (confirmedCandidateIdx !== null && confirmedCandidateIdx >= sortedCandidates.length) {
            setError("確定候補が候補一覧の範囲外です。");
            return;
        }
            ...
            <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as "edit" | "confirm")} className="w-full">
                ...
                <TabsContent value="edit" className="space-y-2">
                    ...
                    <div className="h-[720px] w-full">
                        <PeriodSelector
                            ...
                            onChange={(next) => setSelectedPeriods(next.filter((x) => /_P\d+$/.test(x)))}
                            busyPeriods={[]}
                        />
                    </div>
                </TabsContent>
                ...
                <TabsContent value="confirm" className="space-y-2">
                    <div className="flex items-center justify-between">
                        ...
                        <Button type="button" variant="outline" size="sm" onClick={() => setConfirmedCandidateIdx(null)}>
                            未確定に戻す
                        </Button>
                    </div>
                    <AvailabilityTimeline
                        ...
                        onConfirmCandidate={(idx) => setConfirmedCandidateIdx(idx)}
                    />
